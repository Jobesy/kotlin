...
@Inject constructor(
    ...
    internal class CInteropGist(
        ...
        @Internal
        fun getKlibDependencies() = dependencies.files
            .filter { f -> (f.extension == "klib" || f.isDirectory) && f.exists() }
            .map { f -> TargetedCommonizerDependency(LeafCommonizerTarget(konanTarget), f) }
    ...
    private fun commonize(group: CInteropCommonizerGroup) {
        ...
        val externalDeps = cinteropsForTarget.flatMap { it.getKlibDependencies() }.toSet()
        val nativeDistributionDeps = getNativeDistributionDependencies(group)
        GradleCliCommonizer(commonizerRunner).commonizeLibraries(
            ...
            dependencyLibraries = externalDeps + nativeDistributionDeps,
